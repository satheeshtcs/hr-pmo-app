...
app.put('/tcapture', function (req, res) {
  ...
  con.query("UPDATE task_report SET Value = ? WHERE emp_id = ? AND task_id = ? AND month=?", [Value,emp_id,task_id,month], function (error, results, fields) {
      if (error) throw error;
      return res.send({ error: false, data: results, message: 'Task has been updated successfully.' });
  });
});
app.post('/tcapture1', function (req, res) {
 
  let emp_id = req.body.emp_id;
  let task_id = req.body.task_id;
  let month = req.body.month;

  con.query(" INSERT INTO task_report SET ? ", { emp_id: emp_id ,task_id: task_id,month: month  }, function (error, results, fields) {
      if (error) throw error;
      return res.send({ error: false, data: results, message: 'Task has been updated successfully.' });
  });
});
