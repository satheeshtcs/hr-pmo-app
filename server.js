...
app.put('/complete', function (req, res) {
  ...
  con.query("UPDATE task SET tast_status = '1' WHERE task_id = ?", [task_id], function (error, results, fields) {
      if (error) throw error;
      return res.send({ error: false, data: results, message: 'Task has been updated successfully.' });
  });
});
app.put('/assign1', function (req, res) {
 
  let emp_id = req.body.emp_id;
  
  con.query("UPDATE task_members SET flag = '1' WHERE emp_id = ?", [emp_id], function (error, results, fields) {
      if (error) throw error;
      return res.send({ error: false, data: results, message: 'Task has been updated successfully.' });
  });
});
