...
app.get('/roleMapping3', function (req, res) {
  //Make SQL statement:
  var sql = "SELECT * from user_role_mapping inner join user_role on user_role.role_code = user_role_mapping.role_code";
  //Execute the SQL statement, with the value array:
  con.query(sql, function (err, result) {
    if (err) throw err;
    return res.send({ status: true, data: result, message: 'Data List success' });
...
app.put('/edithierarchy', function (req, res) {
 
  let cemp_id = req.body.cemp_id;
  let pemp_id = req.body.pemp_id;
  

  con.query("UPDATE tag SET parent_id = ?  WHERE child_id = ?", [cemp_id,pemp_id], function (error, results, fields) {
      if (error) throw error;
      return res.send({ error: false, data: results, message: 'Task has been updated successfully.' });
  });
});
